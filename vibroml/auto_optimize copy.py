...
def run_parameter_sweep_optimization(args, output_dir, supercell_ns, deltas, fmax_values, negative_phonon_threshold_thz, soft_mode_max_iterations, soft_mode_displacement_scales, soft_mode_num_top_structures_to_analyze,
                                    phonon_path_npoints, 
                                    phonon_dos_grid,     
                                    default_traj_kT, cell_scale_factors ):
    """
    ...
    for i in range(len(supercell_ns)):
        ...
        phonon_path_npoints=phonon_path_npoints, # Pass from args (which gets from defaults)  
        phonon_dos_grid=phonon_dos_grid,       # Pass from args (which gets from defaults)  
        traj_kT=default_traj_kT                # Pass from args (which gets from defaults)  
            )
    ...
    if best_negative_frequency < threshold_in_current_units and best_softest_mode_info is not None and best_relaxed_atoms is not None:
        ...
        run_soft_mode_optimization(
            ...
            displacement_scales=soft_mode_displacement_scales,  # Default displacement scale factors
            num_top_structures_to_analyze=soft_mode_num_top_structures_to_analyze,  # Default number of structures to analyze
            negative_phonon_threshold_thz=negative_phonon_threshold_thz,  # Default threshold in THz
...
def run_soft_mode_optimization(args, base_output_dir, initial_atoms_for_soft_mode_analysis, initial_softest_mode_info, max_iterations, displacement_scales, num_top_structures_to_analyze, negative_phonon_threshold_thz,
                            phonon_path_npoints, phonon_dos_grid, default_traj_kT, cell_scale_factors):  
    """  
    ...
    for iteration_idx in range(1, max_iterations + 1):
        ...
        if not lowest_energy_structures:
            ...
            break
        # --- Create consolidated summary for this iteration's displaced supercells ---  
        soft_mode_label_for_summary = current_softest_mode_info.get('label', 'unknown')  
        current_mode_folder_path = os.path.join(base_output_dir, f"soft_mode_{iteration_idx}_{soft_mode_label_for_summary}")  
        print(f"\nDEBUG: Attempting to create consolidated summary for iteration {iteration_idx}.")  
        print(f"DEBUG: Expected mode folder path: {current_mode_folder_path}")  
        # Ensure the directory exists before calling the summary function  
        if os.path.exists(current_mode_folder_path):  
            print(f"DEBUG: Mode folder exists: {current_mode_folder_path}")
            from .utils.relaxation_utils import create_displaced_supercell_summary # Import the new function  
            create_displaced_supercell_summary(current_mode_folder_path)  
        else:  
            print(f"Warning: Mode folder {current_mode_folder_path} not found for summary generation.")  
        ...
        for i, result in enumerate(lowest_energy_structures):
            ...
            original_displaced_cif_path = result['original_file'] # This is the path to the displaced CIF
            ...
            # --- NEW: Copy the original displaced CIF file to this analysis folder ---  
            import shutil  
            copied_original_displaced_cif_path = os.path.join(top_structure_phonon_dir, os.path.basename(original_displaced_cif_path))  
            shutil.copy(original_displaced_cif_path, copied_original_displaced_cif_path)  
            print(f"  Copied original displaced CIF to: {copied_original_displaced_cif_path}")  
            # --- END NEW ---
