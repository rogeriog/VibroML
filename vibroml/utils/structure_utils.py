...
def initialize_calculator(engine):
   ...
   if engine == "m3gnet":
      ...
      calculator = mace_mp(model="medium-omat-0",
                           dispersion=False,
...
def generate_displaced_supercells(primitive_atoms, softest_mode_info, supercell_variants, displacement_scales, output_base_dir, iteration_idx, original_prefix):
   """
   ...
   # Define cell parameter scaling factors
   cell_scale_factors = [-0.05, 0.0, 0.05, 0.10]  # -5%, 0%, +5%, +10%
   cell_scale_labels = ["m5", "00", "p5", "p10"]  # Labels for filenames
